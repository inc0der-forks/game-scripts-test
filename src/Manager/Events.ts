        ...
        switch (targetKind) {
            ...
            case 2: // Send to a particular object
                if (targetID === -1) {
                    ...
                    // Send to the hero
                    Game.current.hero.receiveEvent(sender, isSystem, 
                        eventID, parameters, Game.current.heroStates);
                } else {
                    Scene.Map.current.updatePortions(this, function(x: 
                        ...
                        number)
                    {
                        ...
                        if (mapPortion) {
                                ...
                                a++)
                            {
                                ...
                                if (object.system.id === targetID) {
                                    object.receiveEvent(sender, isSystem, 
                                        eventID, parameters, object.states);
                                    break;
                                }
                            }
        ...
        senderNoReceiver: boolean = false, onlyTheClosest: boolean = false)
    {
        ...
        if (!senderNoReceiver || sender !== Game.current.hero) {
            if (targetID !== -1) {
                ...
                if (!Datas.Systems.getDetection(targetID).checkCollision(sender, 
                    Game.current.hero))
                {
                    return;
                }
